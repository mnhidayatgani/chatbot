...
class MessageRouter {
  ...
  async handlePaymentProof(message, customerId) {
    ...
    try {
      ...
      // Forward to admin
      await this.forwardToAdmin(customerId, orderId, cart, totalUSD, filepath);
   ...
   */
  async forwardToAdmin(customerId, orderId, cart, totalUSD, filepath) {
    const adminNumbers = [
  ...
  async handleMessage(message) {
    try {
      ...
      if (
        ...
        const handlers = this.chatbotLogic.handlers;
        const wishlistResponse = await handlers.customer.handleAddToWishlist(
          customerId,
          normalizedMsg
        );
      ...
      if (normalizedMsg.startsWith("hapus ")) {
        ...
        const handlers = this.chatbotLogic.handlers;
        const removeResponse = await handlers.customer.handleRemoveFromWishlist(
          customerId,
          product.id
        );
